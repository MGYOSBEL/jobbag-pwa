...
export class MediaService {
  ...
  editProfileHeaderPicture(userProfileId: number, picture: string): Observable<boolean> {
    const request = {
      client_id: environment.clientId,
      client_secret: environment.clientSecret,
      user_profile_id: userProfileId,
    ...
    return this.http.put<APIResponse>(`${environment.apiBaseURL}/media/userProfilePictureHeader`, request).pipe(
      map(response => {
        const content = JSON.parse(response.content); // Seleccionar la parte del response q es el contenido
        if (response.status_code === 200) {
          return JSON.parse(content); // Retorno el content del response como cuerpo del observable
  ...
  deleteProfileHeaderPicture(userProfileId: number, picture: string): Observable<boolean> {
    const request = {
      client_id: environment.clientId,
      client_secret: environment.clientSecret,
      user_profile_id: userProfileId,
    ...
    return this.http.request<APIResponse>('DELETE', `${environment.apiBaseURL}/media/userProfilePictureHeader`, { body: request }).pipe(
      catchError(err => {
        throw new Error(err.error.status + ': ' + err.error.detail);  // Relanzo el error con el status y el detail
      }),
      map(response => {
